...
def daily_run(context, data):
    ...
    for stock in context.portfolio.positions:
        ...
        if stock in data and 'price' in data[stock]:
           ...
           if price < context.stop_price[stock] and stock not in open_orders:
              #exit when trailing stop hit
              order_target(stock, 0)
              context.stop_price[stock] = 0
            
           if (stock.end_date - get_datetime()).days <= 2 and stock not in open_orders:   
...
def rebalance(context,data):
   ...
   for stock in context.portfolio.positions.iterkeys():
        ...
        if ((context.portfolio.positions[stock].amount* context.portfolio.positions[stock].last_sale_price
